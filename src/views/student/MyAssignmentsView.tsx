...
function findSubjectTitle(subjectId: number | null, courses: Course[], assignmentLabel: string) {
  if (subjectId == null) return assignmentLabel;
...
const ASSIGNMENT_STATUS_KEYS: Record<string, TranslationKey> = {
  not_started: 'student.homework.status.notStarted',
  draft: 'student.homework.status.inProgress',
  submitted: 'student.homework.status.submitted',
  returned: 'student.homework.status.returned',
  graded: 'student.homework.status.graded',
  reading: 'student.books.status.reading',
  completed: 'student.books.status.completed',
};

function homeworkToSelection(homework: HomeworkAssignmentRow, courses: Course[], assignmentLabel: string): HomeworkDetailSelection {
  ...
  const run: SubjectRun = {
    ...
    subjectTitle: findSubjectTitle(homework.subject_id, courses, assignmentLabel),
    course,
    items: [],
  };
...
export function MyAssignmentsView({
      ...
      if (assignmentIds.length > 0) {
        ...
        if (submissionError) {
          ...
          submissions = (submissionRows ?? []).map(row => ({
            ...
            comments: (row.comments ?? []).map(comment => mapHomeworkComment(comment, t('common.unknown'))),
  ...
  }, [currentUser.id, currentUser.name, refreshKey, subjectIds, t]);

  useEffect(() => {
    if (initialHomeworkId == null || loading) return;
    const homework = homeworkAssignments.find(item => item.id === initialHomeworkId);
    if (!homework) return;
    setSelectedHomework(homeworkToSelection(homework, courses, t('student.assignment')));
    onInitialHomeworkHandled?.();
  }, [courses, homeworkAssignments, initialHomeworkId, loading, onInitialHomeworkHandled, t]);

  const rows = useMemo(() => {
    const assignmentLabel = t('student.assignment');
    ...
    const homeworkRows = homeworkAssignments.map(assignment => {
      ...
      return {
        ...
        subtitle: findSubjectTitle(assignment.subject_id, courses, assignmentLabel),
  ...
  }, [bookAssignments, bookSubmissions, courses, homeworkAssignments, homeworkSubmissions, kind, query, status, t]);
    ...
    <div className="space-y-5">
      <div className="border-l-2 border-[#171717] pl-4">
        <div className="grid gap-4 border-b border-[#d4d4d4] pb-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#737373]">{t('student.work.eyebrow')}</p>
            <h1 className="tbo-display mt-1 text-3xl text-[#171717]">{t('student.assignments.title')}</h1>
            <p className="mt-1 text-sm text-[#737373]">{t('student.assignments.intro')}</p>
          </div>
          <div className="flex flex-wrap items-center gap-2 lg:justify-end">
            <span className="inline-flex h-9 items-center gap-2 border-l-2 border-[#c2410c] bg-[#fff7ed] px-3 text-sm font-semibold text-[#c2410c]">{t('student.assignments.open', { count: openCount })}</span>
            <span className="inline-flex h-9 items-center gap-2 border-l-2 border-[#1d4ed8] bg-[#eff6ff] px-3 text-sm font-semibold text-[#1d4ed8]">{t('student.assignments.submitted', { count: submittedCount })}</span>
            <span className="inline-flex h-9 items-center gap-2 border-l-2 border-[#047857] bg-[#ecfdf5] px-3 text-sm font-semibold text-[#047857]">{t('student.assignments.done', { count: doneCount })}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-3 border-y border-[#d4d4d4] bg-white px-4 py-3 lg:grid-cols-[minmax(240px,1fr)_auto_auto] lg:items-center">
        <div className="relative">
          ...
          <input value={query} onChange={event => setQuery(event.target.value)} placeholder={t('student.assignments.search')} className="tbo-focus h-10 w-full border-0 border-b border-[#d4d4d4] bg-transparent pl-7 pr-3 text-sm font-medium text-[#171717]" />
        </div>
        <select value={kind} onChange={event => setKind(event.target.value as AssignmentKindFilter)} className="tbo-focus h-9 rounded-md border border-[#e5e5e5] bg-[#fafafa] px-2 text-sm font-semibold text-[#171717]">
          <option value="all">{t('student.assignments.kind.all')}</option>
          <option value="homework">{t('student.assignments.kind.homework')}</option>
          <option value="reading">{t('student.assignments.kind.reading')}</option>
        </select>
        <select value={status} onChange={event => setStatus(event.target.value as AssignmentStatusFilter)} className="tbo-focus h-9 rounded-md border border-[#e5e5e5] bg-[#fafafa] px-2 text-sm font-semibold text-[#171717]">
          ...
          <option value="not_started">{t('student.homework.status.notStarted')}</option>
          <option value="draft">{t('student.homework.status.inProgress')}</option>
          <option value="submitted">{t('student.homework.status.submitted')}</option>
          <option value="returned">{t('student.books.status.returned')}</option>
          <option value="graded">{t('student.homework.status.graded')}</option>
          <option value="reading">{t('student.books.status.reading')}</option>
          <option value="completed">{t('student.books.status.completed')}</option>
        </select>
      </div>

      <div className="overflow-hidden rounded-2xl border border-[#e5e5e5] bg-white">
        {loading ? (
          <p className="p-6 text-sm text-[#737373]">{t('student.assignments.loading')}</p>
        ) : rows.length === 0 ? (
          <p className="p-8 text-center text-sm text-[#737373]">{t('student.assignments.empty')}</p>
        ) : pagedRows.map(row => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => {
                if (row.assignment) setSelectedHomework(homeworkToSelection(row.assignment, courses, t('student.assignment')));
            ...
            >
              ...
              <span className="text-sm font-semibold text-[#525252]">{row.dueDate ? formatPlatformDate(row.dueDate) : t('common.noDueDate')}</span>
              <span className="flex items-center justify-end gap-3">
                <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${
                  ...
                  {ASSIGNMENT_STATUS_KEYS[row.status] ? t(ASSIGNMENT_STATUS_KEYS[row.status]) : row.status.replace(/_/g, ' ')}
                </span>
      ...
      {!loading && rows.length > ASSIGNMENTS_PER_PAGE && (
        <div className="flex flex-wrap items-center justify-between gap-3 border-y border-[#d4d4d4] bg-white px-4 py-3">
          <p className="text-sm font-semibold text-[#171717]">
            {t('student.assignments.pageRange', {
              from: currentPage * ASSIGNMENTS_PER_PAGE + 1,
              to: Math.min(rows.length, (currentPage + 1) * ASSIGNMENTS_PER_PAGE),
              total: rows.length,
            })}
          </p>
