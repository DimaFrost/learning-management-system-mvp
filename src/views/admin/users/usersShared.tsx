...
export function ActiveYearGroupBadge({ course }: { course: Course }) {
  ...
  const label = isSecond ? t('common.yearGroup.second') : t('common.yearGroup.first');
...
export function OnlineStudentBadge() {
  ...
  return (
    ...
    <span
      ...
      title={t('users.onlineStudent.title')}

    >

      <Wifi className="h-3.5 w-3.5" />

      {t('users.onlineStudent.label')}
...
export function RoleBadges({ roles, yearGroups = [] }: { roles: UserRole[]; yearGroups?: Course[] }) {
  ...
  if (roles.length === 0) {
    ...
    return <span className="text-xs italic text-[#737373]">{t('users.noRoles')}</span>;
