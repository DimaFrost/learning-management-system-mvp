...
const EVENT_TONES: Record<StudentCalendarEventType, { tone: string; icon: typeof Calendar }> = {
...
const STATUS_STYLES: Record<AttendanceStatus, { icon: typeof Check; className: string }> = {
  present: { icon: Check, className: 'text-[#16a34a]' },
  late: { icon: Clock3, className: 'text-[#ea580c]' },
  absent: { icon: X, className: 'text-[#dc2626]' },
};
...
function useStatusMeta() {
  const { t, language } = useLanguage();
  return useMemo(() => ({
    present: { ...STATUS_STYLES.present, label: t('attendance.present') },
    late: { ...STATUS_STYLES.late, label: t('attendance.late') },
    absent: { ...STATUS_STYLES.absent, label: t('attendance.absent') },
  }), [language, t]);
}
...
function formatMonthLabel(date: Date) {
  return formatDateCapitalized(date, { month: 'long', year: 'numeric' });
}
...
function AttendanceStatusIcon({
  ...
  statusMeta: ReturnType<typeof useStatusMeta>;
  notMarkedLabel: string;
}) {
  if (!status) {
    return <span className="h-2 w-2 rounded-full bg-[#d4d4d4]" title={notMarkedLabel} />;
  }
...
function EventRow({
  ...
  eventMeta: ReturnType<typeof useEventMeta>;
  statusMeta: ReturnType<typeof useStatusMeta>;
  notMarkedLabel: string;
}) {
  ...
  const content = (
    <>
      ...
      <AttendanceStatusIcon status={event.status} statusMeta={statusMeta} notMarkedLabel={notMarkedLabel} />
    </>
  );
...
export function StudentMonthCalendar({
  ...
  const weekdays = useMemo(
    () => WEEKDAY_KEYS.map(key => t(`common.weekday.short.${key}`)),
    [language, t],
  );
  ...
  const notMarkedLabel = t('attendance.notMarkedYet');
        ...
        <aside className="min-w-0 rounded-xl border border-[#e5e5e5] bg-white p-4 shadow-[0_1px_0_rgba(0,0,0,0.03)]">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#737373]">
                {tCount('student.calendar.eventCount', selectedEvents.length)}
              </p>
            ...
            <button
              ...
              aria-label={t('student.calendar.closeDay')}
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            {selectedEvents.length === 0 ? (
              <p className="rounded-xl bg-[#f5f5f5] p-4 text-sm text-[#737373] sm:col-span-2">{t('student.calendar.noSessions')}</p>
            ) : (
        ...
        {monthCalendarDays.map(day => {
          ...
          return (
            <button
              ...
              aria-label={t('student.calendar.openDay', { count: dayEvents.length, date: day.date })}
            >
