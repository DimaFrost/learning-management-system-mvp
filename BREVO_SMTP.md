...
<CardGroup>
  ...
  <Card title="Dynamic content" icon="fa-duotone fa-envelope-open-text" href="#send-a-message-with-dynamic-content">
    Send a message enriched with contact attributes or with custom variables.<br></br><br></br>
  </Card>
</CardGroup> 
...
<Info>You can only pick one of the three body types per request.</Info>
...
Pass <Tooltip tip="Dynamic variables can be contact attributes like name, lastname, address">dynamic variables</Tooltip> directly in your API request using the `params` object to personalize the message body. Variables are included in the request payload alongside the email content.
...
<Info>You can only pick one of the two body types per request.</Info>
...
<Note>
Manage contacts via the API using [Create contact attribute](/reference/create-attribute) and [Create a contact](/reference/create-contact). For importing contacts, see [synchronize contact lists](/docs/synchronise-contact-lists).
</Note>
...
<Warning>
The API reference makes real API calls. Check your [rate limits](/docs/api-limits) and [credits](https://account.sendinblue.com/pricing) before testing.
</Warning>
...
<Steps>
  ...
  <Frame background="subtle">
  ![transactional webhooks page](file:docs/assets/images/2d8bbf5-Screenshot_2023-07-11_at_12.50.50.png)
  </Frame>

  <Note>
  Manage webhooks via the API using [Create a webhook](/reference/create-webhook) and [Update a webhook](/reference/update-webhook).
  </Note>
